...
import static com.echopen.asso.echopen.utils.Constants.Http.HEADER_PARSE_APP_ID;
import static com.echopen.asso.echopen.utils.Constants.Http.HEADER_PARSE_REST_API_KEY;
import static com.echopen.asso.echopen.utils.Constants.Http.PARSE_APP_ID;
import static com.echopen.asso.echopen.utils.Constants.Http.PARSE_REST_API_KEY;
...
public class BootstrapService {
     ...
     * @param apiKey
     */
    public BootstrapService(final String apiKey) {
        this.username = null;
        this.password = null;
        this.apiKey = apiKey;
    }
    ...
    private HttpRequest configure(final HttpRequest request) {
        ...
        request.userAgent(System.getProperty("http.agent"));
    ...
    /**
     * Get all bootstrap Laws that exists on NotYet.com
     *
     * @return non-null but possibly empty list of bootstrap
     * @throws IOException
     */
        /*as an example :
        public List<Data> getMyData() throws IOException {
            try {
                HttpRequest request = execute(HttpRequest.get(URL_MY_DATA));
                Reader reader = request.bufferedReader();
                Type type = new TypeToken<List<Data>>() {
                }.getType();
                List<Data> response = new GsonBuilder().create().fromJson(reader, type);
                if (response != null)
                    return response;
