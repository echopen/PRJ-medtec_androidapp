...
public class ScanConversion {
...

    {
        ...
        opencv_src = new Mat(2*rows, cols/2, CvType.CV_8U);
        opencv_src_larger = new Mat(2*larger_rows, cols/2, CvType.CV_8U);
...
//        return null;
    ...
    private int[] opencv_interpolation() throws IOException {
        ...
        if(rows + param2 >= 700) {
            opencv_src.copyTo(opencv_src_larger.submat(0, 2*rows, 0, cols/2));
        }
        else{
            opencv_src.copyTo(opencv_src_larger.submat(param2, 2*rows + param2, 0, cols/2));
        }

        Point center = new Point(128 + param3, 128 + param4);
