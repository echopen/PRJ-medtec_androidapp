...
public class ScanConversion {
...

    {
        ...
        opencv_src = new Mat(rows, 512, CvType.CV_8U);
        opencv_src_larger = new Mat(512, 512, CvType.CV_8U);
...
//        return null;
    ...
    private int[] opencv_interpolation() throws IOException {
        ...
        opencv_src.copyTo(opencv_src_larger.submat(param2, rows + param2, 0, 512));

        Point center = new Point(0, 0);
        Imgproc.linearPolar(opencv_src_larger, opencv_dest, center, 100, Imgproc.INTER_CUBIC + Imgproc.CV_WARP_INVERSE_MAP);
        opencv_dest.convertTo(opencv_dest, CvType.CV_32S);

        int[] dest_out = new int[512 * 512];
